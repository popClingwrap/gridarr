 ...
 * })
 */
export class GridArr {
    _items = [];
    _colCount = Number.POSITIVE_INFINITY;
    _rowCount = 1;
    overflowX = 'none';
    overflowY = 'none';
    /**
    ...
    constructor(config) {
        ...
        if (config.colCount && config.rowCount) {
            ...
            if (config.items.length === 0)
                throw new Error(`Passing colCount without rowCount OR items results in an empty grid with zero dimensions.`);
            this._colCount = config.colCount;
            this._rowCount = Math.ceil(config.items.length / this._colCount);
        }
        else if (config.rowCount) {
            if (config.items.length === 0)
                throw new Error(`Passing rowCount without colCount OR items results in an empty grid with zero dimensions.`);
            this._rowCount = config.rowCount;
        ...
        this._items = sizedArr.map((item, n) => {
            ...
            return new GridCell(item, { x, y }, this, n);
        });
    }
    /**
     ...
     */
    cell(aX, aY, aOverflowX, aOverflowY) {
        const { x, y } = this.convertToValidGridIdRef(aX, aY, aOverflowX, aOverflowY);
        if (x >= this._colCount || y >= this._rowCount)
            throw new Error(`Coordinate {${x}, ${y}} does not exist on this grid. Is wrapping set correctly?`);
    ...
    row(idx) {
        if (idx >= this._rowCount)
            throw new Error(`Row index ${idx} is out of bounds. Only ${this._rowCount} rows exist.`);
     ...
     */
    col(idx) {
        if (idx >= this._colCount)
            throw new Error(`Col index ${idx} is out of bounds. Only ${this._rowCount} cols exist.`);
     ...
     */
    area(aX, aY, aWidth, aHeight, aOverflowX, aOverflowY) {
        aWidth = aWidth < 0 ? aWidth + 1 : aWidth - 1;
        aHeight = aHeight < 0 ? aHeight + 1 : aHeight - 1;
        const { x, y } = this.convertToValidGridIdRef(aX + Math.min(aWidth, 0), aY + Math.min(aHeight, 0), aOverflowX, aOverflowY);
     ...
     * @param aOverflowY - An optional setting for how tyo treat Y coords outside the grid
     */
    convertToValidGridIdRef(aX, aY, aOverflowX, aOverflowY) {
        const tempOverflowX = aOverflowX || this.overflowX;
        const tempOverflowY = aOverflowY || aOverflowX || this.overflowY;
...
class GridCell {
     ...
     */
    constructor(_contents, _gridRef, _grid, _listIndex) {
        this._contents = _contents;
        this._gridRef = _gridRef;
        this._grid = _grid;
     ...
     * @param aOverflowY - An optional setting for how to treat Y coords outside the grid
     */
    relative(aX, aY, aOverflowX, aOverflowY) {
 ...
 */
export function visualise(target, highlights) {
